  ...
  template: `
    <div class="list-container">
      <div class="list-wrapper">
        ...
        <div class="list-header">
          <div class="header-left">
            ...
            <div>
              ...
              <p>{{ store.properties().length }} propiedades registradas</p>
            </div>
          </div>
        ...
        <!-- Empty state -->
        <div *ngIf="!store.loading() && store.properties().length === 0" class="empty-state">
          <div class="empty-icon">🏢</div>
          <h3>No se encontraron propiedades</h3>
          <p>Comienza registrando una nueva propiedad en el sistema</p>
        ...
        <!-- Properties Grid -->
        <div class="properties-grid" *ngIf="!store.loading() && store.properties().length > 0">
          <div
            class="property-card"
            *ngFor="let property of store.properties(); trackBy: trackById"
          >
            <!-- Card Header -->
            <div class="card-header">
              ...
              <div class="card-info">
                ...
                <span class="location-text">{{ property.fullLocation }}</span>
              </div>
            </div>
            ...
            <div class="card-body">
              <div class="detail-row">
                ...
                <span class="detail-value highlight">{{ property.region }}</span>
              </div>
              <div class="detail-row">
                <span class="detail-label">Distrito</span>
                <span class="detail-value">{{ property.district }}</span>
              </div>
              <div class="detail-row" *ngIf="property.photos && property.photos.length > 0">
                <span class="detail-label">Fotos</span>
                <span class="detail-value photo-badge">{{ property.photos.length }} adjuntas</span>
              </div>
            </div>

            <!-- Card Actions -->
            <div class="card-actions">
              <button class="btn-action edit" (click)="goToEdit(property.id)">
                ✏️ Editar
              </button>
              <button class="btn-action delete" (click)="confirmDelete(property)">
                🗑️ Eliminar
              </button>
            </div>
          </div>
        ...
        <div class="modal-overlay" *ngIf="propertyToDelete()" (click)="cancelDelete()">
          <div class="modal-card" (click)="$event.stopPropagation()">
            <div class="modal-icon">⚠️</div>
            <h3>¿Eliminar propiedad?</h3>
            <p>Estás a punto de eliminar la propiedad en <strong>{{ propertyToDelete()?.address }}</strong>. Esta acción no se puede deshacer.</p>
...
export class PropertyListComponent implements OnInit {
  ...
  trackById(_: number, property: Property): string | number {
    return property.id;
  }
